...
The user expanded the scope and rebalanced remaining work on 2026-09-20 to approximately **35% A / 35% B / 30% C**. The [A/B/C assignments](HACKATHON_ASSIGNMENTS.md) and [work board](WORKSTREAMS.md) supersede the old freeze sequence and optional-only C role. Refreshed main at `2e76537` includes the complete cash-gap workflow/C11 demo, C10 kit, and multi-dimension uncertainty controls. C12's dedicated failure view is still required. Preserve these merged deliveries rather than restarting them.
...
1. **Idea #2: explain the minimum cash change.** Generalize the existing offline diagnostic to a private, nonmutating fixed-schedule workflow with proof-qualified hypothetical opening cash, limiting dates and evidence. Money cannot repair missing authorization or evidence.
2. **Idea #3: uncertainty explorer.** Expose the supported amount/date/approval dimensions and multiple incomes, with case-count warnings and understandable failure exploration. Every cent counts; large or incomplete domains remain honestly unknown.
...
A owns engine/contracts/generated types; B leads UI state/forms/shared integration; C implements the uncertainty failure view and consequence walkthrough and delivers demo materials/tests/rehearsals. Merge A contracts before B/C consumption. Each bounded task gets its own handoff and fresh worktree. [C10-C15](DEV_C.md) replaces the old limited review queue, with exact paths and shared-file releases in the existing dev-b lane. C10/C11 are delivered; C12-C15 remain ordered work. A's [synthesis design](RESILIENT_PLAN_SPEC.md) is proposed for B review; synthesis and consequence walkthrough remain unimplemented. C validates its deliverables; A/B mutual review remains required. No successful live inference or human rehearsal is claimed.
...
Current delivery/checks are in [RESUME.md](RESUME.md); [verification semantics](handoffs/verification.md) and [integration history](handoffs/integration.md) remain historical evidence. Shared docs are A-owned; each developer records only its own task handoff.
