...
Hosted NVIDIA is the default and needs no GPU VM. Brev credits fund your organization's GPU compute/storage; they do not configure ClauseGraph's hosted API credential. The optional adapter has mocked tests and a bounded live retest on the user's existing H100. On 2026-09-20 UTC, two runs of the same five synthetic native-text clauses passed citation/literal validation for all five; exact structured fields passed 3/5, then 5/5 without a code change. All results remained withheld pending review. This establishes working inference, not consistent or general extraction accuracy; see the [live retest record](handoffs/dev-a/brev-live-retest.md).
...
Schema-constrained output does not establish factual accuracy. The adapter still validates responses locally, preserves source text, rejects malformed/incomplete output and requires the existing evidence, review and approval gates. It does not repair model claims or fall back to hosted text inference. The [source-reference handoff](handoffs/dev-a/brev-source-references.md) records implementation checks and the earlier live failures; the [restarted-GPU retest](handoffs/dev-a/brev-live-retest.md) records both subsequent outcomes. Neither is an OCR or general model-accuracy claim.

After restarting an existing VM, wait for both VM readiness and model readiness. A manually created container with `--restart=no` needs `docker start clausegraph-nim` inside that VM. The tested restart loaded cached files and warmed kernels for several minutes before `/v1/models` succeeded. Run `brev refresh`, reconnect the private port forward, then check the model endpoint before evaluating; a RUNNING VM alone does not mean inference is ready.
