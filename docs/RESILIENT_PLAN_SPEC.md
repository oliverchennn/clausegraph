...
Reuse the nominal planner's limits of 100 actions and 10000 action/date options. Exceeding a work/representation limit produces `INCONCLUSIVE`/`MODEL_LIMIT`, never proof of impossibility. Do not pre-expand large cent/date products. Domain construction must account for its work and check the deadline. No sampling or monotonic/endpoint shortcut is included in v1.
