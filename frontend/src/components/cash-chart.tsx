...
export default function CashChart({ plan }: { plan: PlanResult }) {
  const baseline = new Map(plan.baseline.daily.map(day => [day.date, day.balance_cents]));
  const data = plan.proposed.daily.map(day => ({ date: day.date, proposed: day.balance_cents, baseline: baseline.get(day.date) ?? 0 }));
  return <div className="cash-chart" role="img" aria-label={`Cash projection. Baseline minimum ${money(plan.baseline.minimum_balance_cents)}; proposed minimum ${money(plan.proposed.minimum_balance_cents)}.`}>
    <ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 12, right: 18, left: 1, bottom: 2 }}>
      ...
      <Tooltip labelFormatter={value => shortDate(String(value))} formatter={(value, name) => [money(Number(value), true), name === "proposed" ? "Proposed plan" : "Current path"]} contentStyle={{ border: "1px solid #e7eaf1", borderRadius: 12, boxShadow: "0 8px 30px #14204610", fontSize: 12 }} />
