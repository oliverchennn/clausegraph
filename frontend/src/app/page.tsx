...
type Comparison = { label: string; summary: string; result: PlanResult };
...
export default function Home() {
  ...
  async function previewAction(action: Action) {
    ...
    const result = await request<PlanResult>("/plan/preview", workspace.session_id, { method: "POST", body: JSON.stringify(body) });
    setComparison({ label: action.title, summary: `${action.title} is the only option applied`, result });
  ...
  async function upload() {
    ...
    form.append("consent_provider", workspace.providers?.some(provider => provider.name === "Gemini") ? "gemini" : "nvidia");
  ...
  function actionCard(action: Action, index: number) {
    ...
    return <article className={`action-card ${chosen ? "action-selected" : risky ? "action-risk" : ""}`} key={action.id} data-testid={`action-${action.id}`}>
      ...
      <div className="action-content"><div className="action-card-top"><span className="eyebrow">{chosen ? `Step ${chosen.order}` : risky ? "Hidden consequence" : "Needs confirmation"}</span><Badge tone={chosen ? chosen.conditional ? "warning" : "success" : risky ? "danger" : "warning"}>{chosen ? chosen.conditional ? "Conditional" : plan.assumptions?.force_action_ids?.includes(action.id) ? "Requested option" : "In your plan" : risky ? "Avoid for now" : "Not in plan"}</Badge></div><h3>{action.title}</h3><p>{chosen?.explanation || action.description}</p>{excluded && <p className="action-exclusion">{excluded}</p>}<div className="action-card-bottom"><button className="text-button" onClick={() => setEvidence(action.source_rule_ids)}><FileText size={13} /> View evidence <ChevronRight size={13} /></button>{chosen ? <span className="action-date"><CalendarDays size={13} /> {shortDate(chosen.execution_date)}</span> : <span className="action-date">{action.approval_status === "pending" ? "Approval pending" : humanize(action.approval_status || "not_required")}</span>}<Button variant="ghost" busy={busy === `compare-${index}`} onClick={() => void run(`compare-${index}`, () => previewAction(action))}>Compare option alone</Button><Button variant="ghost" busy={busy === `draft-${index}`} onClick={() => void run(`draft-${index}`, async () => setDraft(await request<DraftResponse>(`/actions/${encodeURIComponent(action.id)}/draft`, workspace.session_id, { method: "POST" })))}>Draft request <ArrowUpRight size={13} /></Button></div></div>
    </article>;
  }
