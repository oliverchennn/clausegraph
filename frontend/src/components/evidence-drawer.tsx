...
import { dollars, humanize, parseCents, shortDate } from "@/lib/api";
...
function RuleEditor({ rule, workspace, onSave }: { rule: Rule; workspace: Workspace; onSave: (id: string, review: RuleReview) => Promise<void> }) {
  ...
  async function save() {
    ...
    try { await onSave(rule.id, { review_status: review, approval_status: approval, ...(amount ? { amount_cents: parseCents(amount) } : {}), ...(date ? { due_date: date } : {}), note: note || null }); setSaved(true); }
  ...
  return <section className="rule-detail" data-testid={`rule-${rule.id}`}>
    ...
    {rule.conditions?.length ? <ul className="condition-list">{rule.conditions.map((condition, index) => <li key={index}>{condition.resolved && condition.satisfied ? <Check size={15} className="text-green" /> : <CircleHelp size={15} className="text-amber" />}<span>{humanize(condition.fact)} {condition.operator} {String(condition.value ?? "")}<small>{condition.resolved ? condition.satisfied ? "Satisfied" : "Not satisfied" : "Unresolved — excluded from confirmed plans"}</small></span></li>)}</ul> : <p className="helper">No additional conditions recorded.</p>}
