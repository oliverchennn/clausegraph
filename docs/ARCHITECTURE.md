...
`backend/clausegraph/schemas.py` owns API shapes; FastAPI OpenAPI is exported to `docs/openapi.json` and openapi-typescript generates `frontend/src/lib/api-types.ts`. Frontend imports aliases from generated schemas. All money is integer cents, all dates ISO dates. Evidence offsets are page-local, end-exclusive.
