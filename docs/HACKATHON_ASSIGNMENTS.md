...
- Fetched base is `e65464c` (PR24), including PR19-23. A/B's first tasks are published as open PR25/28; C's previous queue is complete per the user, with PR26/27 still awaiting integration. Do not repeat completed work or confuse published with merged. Preserve A's active closeout correctness repair and all peer handoffs. See [WORKSTREAMS](WORKSTREAMS.md) for exact observed heads and integration limits.
...
The initial A/B closeout tasks are now published in PR25/28, not new assignments to repeat. The descriptions below retain their acceptance obligations. A's ongoing continuation includes an observed failed-extraction/empty-plan confirmation blocker; finish its owned repair and checks. B's consent work awaits integration. C's prior PR26/27 remain preserved follow-ups. Human delivery remains outstanding; published preparation is not live success.
