...
**Prepared consumption scope:** A's [explorer specification](UNCERTAINTY_EXPLORER.md) maps the existing result to a bounded witness table/details and separate worst permitted cash result. It specifies exact or explicitly qualified large counts and records B's exact future paths. No backend field addition is needed for that view. This specification can be reviewed while stage 1 UI/C11 finish; its publication does not open stage 2 implementation early or release B's existing files to C. B's merged typed interface and C12 wiring release are still required.
