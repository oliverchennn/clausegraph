...
Developer A owns backend/integration work. Developer B owns frontend work in a separate session/clone. Developer C is a separate, part-time contributor for optional proofreading, bug reproduction and narrowly assigned UI cleanup; read [docs/DEV_C.md](docs/DEV_C.md) before C work. Do not spawn B or C implementation agents or take over their assignments from A's session. Stop after the assigned task; the ordered roadmap is not authorization to implement the remaining backlog.
