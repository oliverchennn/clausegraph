...
export function Modal({ open, onClose, title, description, children, wide = false, drawer = false }: { open: boolean; onClose: () => void; title: string; description?: string; children: ReactNode; wide?: boolean; drawer?: boolean }) {
  ...
  return <Dialog.Root open={open} onOpenChange={value => !value && onClose()}><Dialog.Portal><Dialog.Overlay className="modal-overlay" /><Dialog.Content
    className={clsx("modal", { "modal-wide": wide, drawer })}
    onOpenAutoFocus={() => {
      // These controlled dialogs open from buttons outside a Radix Dialog.Trigger.
