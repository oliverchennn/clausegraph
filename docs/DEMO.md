...
Delivery checkpoint: [`a00af57`](DELIVERY_CHECKPOINT.md) includes merged PR20/PR21 and green combined CI. B's [task 5 handoff](handoffs/dev-b/demo-rehearsal.md) records a 180.02-second automated operator run with narration holds, local fallback/outage recovery and focused desktop/mobile sign-off. Human spoken delivery remains unmeasured; perform that rehearsal separately before presentation sign-off. B's active history UI is not part of this baseline, and no global feature freeze is declared.

The [checkpoint procedure](DELIVERY_CHECKPOINT.md#reproduce-and-prepare-the-local-fallback) explains how to preserve an isolated demo and regenerate the saved synthetic reports without an app/API dependency. Preload them before presenting; if switching to fallback, identify them as previously computed synthetic results. This readiness task did not restart the user's local app. Keep `TEXT_PROVIDER=nvidia` for browser text processing until a separately assigned Brev consent UI is implemented; the synthetic story itself needs no provider calls.
