...
from .schemas import (
    DependencyGraph, Document, GraphEdge, GraphIssue, GraphNode, Rule, Scenario,
)
...
def dependency_issues(scenario: Scenario, rules: list[Rule]) -> list[GraphIssue]:
    ...
    for label, values in (("rule", rules), ("action", scenario.actions), ("event", scenario.events)):
        ids = [value.id for value in values]
        if len(ids) != len(set(ids)):
            issues.append(GraphIssue(code="duplicate_id", message=f"Duplicate {label} identifiers are ambiguous.", rule_ids=list(rule_map)))
        ...
        for rule in rules:
            for target in getattr(rule, relation):
                if target not in rule_map:
                    issues.append(GraphIssue(code="missing_rule", message=f"{rule.title} references missing rule {target}.", rule_ids=[rule.id]))
                else:
                    graph.add_edge(rule.id, target)
        for component in nx.strongly_connected_components(graph):
            if len(component) > 1 or any(graph.has_edge(node, node) for node in component):
                issues.append(GraphIssue(code="rule_cycle", message=f"Unsupported {relation} cycle requires review.", rule_ids=sorted(component)))
    action_graph = nx.DiGraph()
    action_graph.add_nodes_from(action_map)
    for action in scenario.actions:
        for target in action.requires:
            if target not in action_map:
                issues.append(GraphIssue(code="missing_action", message=f"{action.title} requires missing action {target}.", rule_ids=action.source_rule_ids))
            else:
                action_graph.add_edge(action.id, target)
        for target in action.excludes:
            if target not in action_map:
                issues.append(GraphIssue(code="missing_action", message=f"{action.title} excludes missing action {target}.", rule_ids=action.source_rule_ids))
        overlap = set(action.requires) & set(action.excludes)
        if overlap:
            issues.append(GraphIssue(code="contradictory_dependency", message=f"{action.title} both requires and excludes {', '.join(sorted(overlap))}.", rule_ids=action.source_rule_ids))
    for component in nx.strongly_connected_components(action_graph):
        if len(component) > 1 or any(action_graph.has_edge(node, node) for node in component):
            affected = sorted({rid for aid in component for rid in action_map[aid].source_rule_ids})
            issues.append(GraphIssue(code="action_cycle", message="Unsupported action dependency cycle requires review.", rule_ids=affected))
    groups: dict[tuple, list[Rule]] = defaultdict(list)
    for rule in rules:
        if rule.entity_ambiguous:
            issues.append(GraphIssue(code="ambiguous_entity", message=f"The party or account for {rule.title} needs confirmation.", rule_ids=[rule.id]))
    ...
    for group in groups.values():
        for left, right in combinations(group, 2):
            if left.id in right.supersedes or right.id in left.supersedes:
                continue
            amounts_conflict = left.amount_cents is not None and right.amount_cents is not None and left.amount_cents != right.amount_cents
            dates_conflict = left.due_date is not None and right.due_date is not None and left.due_date != right.due_date
            if amounts_conflict or dates_conflict:
                issues.append(GraphIssue(code="contradictory_clauses", message=f"Conflicting amounts or dates for {left.title} need explicit supersession or review.", rule_ids=[left.id, right.id]))
    return issues
