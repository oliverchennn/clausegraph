...
function RuleEditor({ rule, workspace, onSave }: { rule: Rule; workspace: Workspace; onSave: (id: string, review: RuleReview) => Promise<void> }) {
  ...
  return <section className="rule-detail" data-testid={`rule-${rule.id}`}>
    ...
    {Array.from(new Map(rule.evidence.map(item => [`${item.document_id}:${item.version ?? 1}`, item])).values()).map(item => <Button key={`${item.document_id}:${item.version ?? 1}`} className="source-download" onClick={async () => { try { const source = await requestBlob(`/documents/${encodeURIComponent(item.document_id)}/original?version=${item.version ?? 1}`, workspace.session_id); download(source, workspace.documents.find(document => document.id === item.document_id)?.name || "original-document"); } catch (caught) { setError(caught instanceof Error ? caught.message : "Original could not be downloaded."); } }}>Download original source</Button>)}
...
export default function EvidenceDrawer({ ruleIds, workspace, onClose, onSave }: { ruleIds: string[]; workspace: Workspace; onClose: () => void; onSave: (id: string, review: RuleReview) => Promise<void> }) {
  const rules = [...new Set(ruleIds)].map(id => workspace.rules.find(rule => rule.id === id)).filter((rule): rule is Rule => !!rule);
