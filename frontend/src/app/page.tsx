...
import type { Action, DraftResponse, IntakeRequest, PlanRequest, PlanResult, ProviderStatus, RuleReview, UploadResponse, Workspace } from "@/lib/types";
...
type Tab = "overview" | "documents" | "graph";
...
export default function Home() {
  ...
  const openSession = useCallback(async (demo: boolean) => {
    const data = await request<Workspace>("/sessions", undefined, { method: "POST", body: JSON.stringify({ demo }) });
    localStorage.setItem(SESSION_KEY, data.session_id);
    setDeleted(false); setApproval("recorded"); setCash(""); setIncomeDate(""); setActionDate("");
    await withPlan(data);
  }, [withPlan]);
  ...
  async function saveIntake(value: IntakeRequest) {
    ...
    await withPlan(data); setCash(""); setNotice("Financial picture updated. Your plan has been recalculated.");
  }
  ...
  async function saveReview(id: string, review: RuleReview) {
    ...
    await withPlan(data); setApproval("recorded"); setNotice("Review recorded. Your plan now reflects the updated evidence and approval.");
  }

  async function calculate() {
    if (!workspace) return;
    const shift = workspace.scenario.actions.find(action => action.kind === "shift");
    const body: PlanRequest = { include_conditional: false, ...(cash ? { opening_balance_cents: parseCents(cash) } : {}), ...(incomeDate ? { income_date: incomeDate } : {}), ...(shift && actionDate ? { action_dates: { [shift.id]: actionDate } } : {}), ...(shift && approval !== "recorded" ? { approval_overrides: { [shift.id]: approval as "approved" | "denied" | "pending" }, include_conditional: approval === "approved" } : {}) };
  ...
  async function upload() {
    ...
    const form = new FormData(); form.append("file", file); form.append("consent", String(consent));
    const response = await request<UploadResponse>("/documents", workspace.session_id, { method: "POST", body: form });
    await withPlan(await request<Workspace>("/workspace", workspace.session_id));
    setDialog(null); setTab("documents"); setFile(null); setConsent(false);
    setNotice(response.duplicate ? "This document is already in your workspace. No duplicate was created." : "Document uploaded. Processing progress will appear below.");
  }
  ...
  function actionCard(action: Action, index: number) {
    ...
    return <article className={`action-card ${chosen ? "action-selected" : risky ? "action-risk" : ""}`} key={action.id} data-testid={`action-${action.id}`}>
      <div className={`action-symbol ${chosen ? "green" : risky ? "rose" : "amber"}`}>{chosen ? <CalendarDays size={20} /> : risky ? <GitBranch size={20} /> : <CircleHelpIcon />}</div>
      <div className="action-content"><div className="action-card-top"><span className="eyebrow">{chosen ? `Step ${chosen.order}` : risky ? "Hidden consequence" : "Needs confirmation"}</span><Badge tone={chosen ? chosen.conditional ? "warning" : "success" : risky ? "danger" : "warning"}>{chosen ? chosen.conditional ? "Conditional" : "In your plan" : risky ? "Avoid for now" : "Not in plan"}</Badge></div><h3>{action.title}</h3><p>{chosen?.explanation || action.description}</p>{excluded && <p className="action-exclusion">{excluded}</p>}<div className="action-card-bottom"><button className="text-button" onClick={() => setEvidence(action.source_rule_ids)}><FileText size={13} /> View evidence <ChevronRight size={13} /></button>{chosen ? <span className="action-date"><CalendarDays size={13} /> {shortDate(chosen.execution_date)}</span> : <span className="action-date">{action.approval_status === "pending" ? "Approval pending" : humanize(action.approval_status || "not_required")}</span>}<Button variant="ghost" busy={busy === `compare-${index}`} onClick={() => void run(`compare-${index}`, async () => { const compared = await request<PlanResult>("/plan", workspace.session_id, { method: "POST", body: JSON.stringify({ include_conditional: false, force_action_ids: [action.id] }) }); setWorkspace({ ...workspace, plan: compared }); setNotice(`Comparing ${action.title}. This scenario has not executed any action.`); })}>Compare this option</Button><Button variant="ghost" busy={busy === `draft-${index}`} onClick={() => void run(`draft-${index}`, async () => setDraft(await request<DraftResponse>(`/actions/${encodeURIComponent(action.id)}/draft`, workspace.session_id, { method: "POST" })))}>Draft request <ArrowUpRight size={13} /></Button></div></div>
    </article>;
  }

  return <div className="app-shell">
    <aside className="sidebar"><Link className="brand" href="/" aria-label="ClauseGraph home"><span className="brand-mark"><GitBranch size={24} strokeWidth={2.1} /></span><span>Clause<span className="brand-light">Graph</span><small>CLARITY INTO ACTION</small></span></Link><div className="workspace-switch"><span className="workspace-avatar">P</span><div>Personal workspace<small>Financial recovery</small></div><ChevronDown size={14} /></div><div className="nav-label">WORKSPACE</div><nav><button className={tab === "overview" ? "nav-item active" : "nav-item"} onClick={() => setTab("overview")}><LayoutDashboard size={18} /> Overview<span className="nav-active-dot" /></button><button className={tab === "documents" ? "nav-item active" : "nav-item"} onClick={() => setTab("documents")}><FileText size={18} /> Your documents{workspace && <span className="nav-count">{workspace.documents.length}</span>}</button><button className={tab === "graph" ? "nav-item active" : "nav-item"} onClick={() => setTab("graph")}><GitBranch size={18} /> Dependency graph</button><button className="nav-item" disabled={!workspace} onClick={() => setDialog("intake")}><Wallet size={18} /> Financial picture</button></nav><div className="sidebar-note"><div className="sidebar-note-icon"><ShieldCheck size={21} /></div><h3>Your essentials come first.</h3><p>Every option is checked against the expenses you need to protect.</p><button onClick={() => setDialog("intake")} disabled={!workspace}>Review essentials <ArrowRight size={14} /></button></div><div className="sidebar-bottom"><button className="nav-item" disabled={!workspace} onClick={() => setDialog("settings")}><Settings2 size={18} /> Settings & privacy</button><div className="profile"><div className="profile-avatar">Y</div><div>Your workspace<small><span className="status-dot" /> Private session</small></div><LockKeyhole size={15} /></div></div></aside>
    <main className="main"><header className="topbar"><div className="breadcrumb">Workspace <ChevronRight size={13} /> <span>{tab === "overview" ? "Overview" : tab === "documents" ? "Your documents" : "Dependency graph"}</span></div><div className="topbar-right"><span className="private-label"><LockKeyhole size={13} /> Private by design</span>{workspace?.mode === "synthetic" && <Badge tone="warning"><span className="mini-dot" /> Synthetic demo</Badge>}<button className="top-avatar" aria-label="Open settings" onClick={() => setDialog("settings")} disabled={!workspace}>Y</button></div></header>
      <div className="page-content">
        <div className="page-heading"><div><div className="eyebrow page-eyebrow">A LITTLE CLARITY. A WAY FORWARD.</div><h1>{tab === "overview" ? "Your recovery plan" : tab === "documents" ? "The evidence behind your plan" : "See how it all connects"}</h1><p>{tab === "overview" ? "Understand your options. Protect what matters. Take the next step." : tab === "documents" ? "Your documents, extracted facts, and the fine print that changes the picture." : "Follow the relationships between documents, clauses, actions, and your cash."}</p></div><Button variant="primary" disabled={!workspace || !!busy} onClick={() => setDialog("upload")}><Plus size={17} /> Add documents</Button></div>
        {workspace?.mode === "synthetic" && <div className="demo-strip"><Sparkles size={15} /><span>You’re exploring a synthetic scenario. All six example documents and financial details are fictional.</span><button onClick={() => void run("new", () => openSession(false))} disabled={!!busy}>Start my own plan <ArrowRight size={13} /></button></div>}
        ...
        {loading ? <div className="loading-workspace"><span className="loading-logo"><GitBranch size={32} /></span><h2>Connecting the dots…</h2><p>Opening your clearly labeled synthetic demo.</p><LoaderCircle className="spin" size={20} /></div> : !workspace ? <div className="welcome-card"><div className="large-icon"><ShieldCheck size={28} /></div><h2>{deleted ? "Your session has been deleted" : "Let’s find your next step"}</h2><p>{deleted ? "Your documents, extracted rules, and financial data were removed from this session." : "Open the synthetic example or start a private plan with your own financial picture."}</p><div className="button-row"><Button variant="primary" busy={busy === "new"} onClick={() => void run("new", () => openSession(false))}>Start a new plan</Button><Button busy={busy === "demo"} onClick={() => void run("demo", () => openSession(true))}>Explore synthetic demo</Button></div></div> : <>
          ...
          <Tabs.Content value="documents" className="tab-content"><div className="documents-heading"><div><h2>Your document library <span className="count-pill">{workspace.documents.length}</span></h2><p>Source text stays separate from reviewed facts and third-party approvals.</p></div><Button onClick={() => setDialog("upload")}><Upload size={15} /> Upload document</Button></div><div className="document-grid">{workspace.documents.map(document => { const rules = workspace.rules.filter(rule => rule.evidence.some(source => source.document_id === document.id)); const job = workspace.jobs?.find(item => item.document_id === document.id && item.status !== "completed"); return <article className="document-card" key={document.id}><div className="document-card-top"><span className="document-icon"><FileText size={24} /></span><Badge tone={document.status === "ready" ? "success" : document.status === "failed" ? "danger" : "warning"}>{humanize(document.status)}</Badge></div><h3>{document.name}</h3><p>{document.pages?.length || 0} pages · Version {document.version}{document.synthetic && " · Synthetic"}</p>{job && <div className={`job-status ${job.status === "failed" ? "job-failed" : ""}`}><span>{job.status === "failed" ? <CircleAlert size={13} /> : <LoaderCircle size={13} className="spin" />}{humanize(job.stage)} · {job.progress}%</span><progress value={job.progress} max={100} />{job.error && <p role="alert">{job.error}</p>}</div>}{document.error && <p className="text-red">{document.error}</p>}<div className="document-card-footer"><button className="text-button" onClick={() => setEvidence(rules.map(rule => rule.id))}>{rules.length} extracted facts <ArrowRight size={13} /></button><button className="icon-button" aria-label={`Delete ${document.name}`} onClick={() => { if (window.confirm(`Delete ${document.name} and invalidate its recommendations?`)) void run("delete-document", async () => { await request(`/documents/${encodeURIComponent(document.id)}`, workspace.session_id, { method: "DELETE" }); await withPlan(await request<Workspace>("/workspace", workspace.session_id)); }); }}><Trash2 size={14} /></button></div></article>; })}<button className="document-upload-card" onClick={() => setDialog("upload")}><span><Plus size={24} /></span><strong>Add another piece of the picture</strong><p>Bills, contracts, or benefit documents</p><small>PDF, TXT, or CSV</small></button></div><section className="panel facts-panel"><div className="panel-heading"><div><h2>Extracted facts & review</h2><p>Open a fact to inspect its exact source and confirm what it means.</p></div><Badge tone="warning">{pendingReviews} need review</Badge></div><div className="facts-table-wrapper"><table className="facts-table"><thead><tr><th>Fact</th><th>Evidence</th><th>Human review</th><th>Approval</th><th /></tr></thead><tbody>{workspace.rules.map(rule => <tr key={rule.id}><td><button onClick={() => setEvidence([rule.id])}>{rule.title}</button><small>{rule.amount_cents != null ? money(rule.amount_cents) : humanize(rule.kind)}{rule.due_date && ` · ${shortDate(rule.due_date)}`}</small></td><td><Badge tone={rule.evidence_status === "supported" ? "success" : "warning"}>{rule.evidence_status}</Badge></td><td>{humanize(rule.review_status || "pending")}</td><td><Badge tone={rule.approval_status === "approved" ? "success" : rule.approval_status === "denied" ? "danger" : "neutral"}>{humanize(rule.approval_status || "not_required")}</Badge></td><td><button className="icon-button" aria-label={`Review ${rule.title}`} onClick={() => setEvidence([rule.id])}><ChevronRight size={17} /></button></td></tr>)}</tbody></table>{!workspace.rules.length && <div className="empty-small">No extracted facts yet. Upload a document to get started.</div>}</div></section></Tabs.Content>
          <Tabs.Content value="graph" className="tab-content"><section className="panel graph-panel"><div className="panel-heading"><div><h2>The bigger picture</h2><p>Click any clause or connection to inspect its evidence. Drag to explore; scroll to zoom.</p></div><Badge tone="blue">{workspace.graph.nodes?.length ?? 0} connections of context</Badge></div><div className="graph-legend"><span><i style={{ background: "#b9cafa" }} /> Documents</span><span><i style={{ background: "#a9d9c6" }} /> Rules</span><span><i style={{ background: "#eed09b" }} /> Actions</span><span><i style={{ background: "#bfc9df" }} /> Cash events</span></div><DependencyGraph graph={workspace.graph} onEvidence={setEvidence} /></section>{workspace.graph.issues?.map((issue, index) => <div className="alert graph-issue" key={index}><CircleAlert size={18} /><div><strong>{humanize(issue.code)}</strong><p>{issue.message}</p></div><button className="text-button" onClick={() => setEvidence(issue.rule_ids ?? [])}>Review evidence <ArrowRight size={14} /></button></div>)}</Tabs.Content></Tabs.Root>
    ...
    {dialog === "upload" && <Modal open onClose={() => setDialog(null)} title="Add your documents" description="A bill, a contract, or a benefits letter can change the picture."><div className="dialog-body"><button className="upload-dropzone" onClick={() => uploadInput.current?.click()} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); setFile(event.dataTransfer.files[0] || null); }}><span><Upload size={27} /></span><strong>{file ? file.name : "Choose a document or drop it here"}</strong><p>{file ? `${Math.ceil(file.size / 1024)} KB · Ready to upload` : "PDF, TXT, or CSV · Up to 10 MB"}</p></button><input ref={uploadInput} className="sr-only" type="file" accept=".pdf,.txt,.csv,application/pdf,text/plain,text/csv" onChange={event => setFile(event.target.files?.[0] || null)} /><label className="consent-box"><input type="checkbox" checked={consent} onChange={event => setConsent(event.target.checked)} /><span>I consent to external document processing.<small>Document content and your scenario’s financial facts may be sent to NVIDIA and Google for extraction and verification. Where configured, originals are stored in private cloud storage. Without consent, external processing will not run.</small></span></label><div className="privacy-note"><LockKeyhole size={15} /><p>Documents stay in your private session. You can delete them and their derived data at any time.</p></div>{error && <div className="inline-error" role="alert">{error}</div>}</div><div className="dialog-footer"><Button onClick={() => setDialog(null)}>Cancel</Button><Button variant="primary" disabled={!file || file.size > 10 * 1024 * 1024} busy={busy === "upload"} onClick={() => void run("upload", upload)}><Upload size={15} /> Upload document</Button></div></Modal>}
    {draft && <Modal open onClose={() => setDraft(null)} title="Your request, ready to review" description="A draft only. Nothing has been sent or submitted."><div className="dialog-body"><Badge tone="blue">Not sent</Badge><Field label="Subject"><input value={draft.subject} onChange={event => setDraft({ ...draft, subject: event.target.value })} /></Field><Field label="Message"><textarea rows={12} value={draft.body} onChange={event => setDraft({ ...draft, body: event.target.value })} /></Field><p className="helper">Check the details and send it yourself when you’re ready. Approval is only recorded when you explicitly update the source review.</p></div><div className="dialog-footer"><Button onClick={() => { download(new Blob([`${draft.subject}\n\n${draft.body}`], { type: "text/plain" }), "clausegraph-request.txt"); }}>Download draft <Download size={14} /></Button><Button variant="primary" onClick={() => void run("copy", async () => { await navigator.clipboard.writeText(`${draft.subject}\n\n${draft.body}`); setNotice("Draft copied. Nothing has been sent."); setDraft(null); })}><ClipboardList size={15} /> Copy draft</Button></div></Modal>}
